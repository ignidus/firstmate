...
For every open story, either take its next executable lifecycle action, check and steer work already in flight, place it behind a genuine dependency, or hold it because it needs John's decision.
...
Before dispatching, editing Jira, or creating any crewmate, classify every open story into exactly one of the four Objective conditions above using your own technical reading, without acting on any of them.
...
- count and list of story keys in each of the four conditions;
...
4. mark or retain the story as `hold_kind: captain` according to the governing workflow;
5. add it to the Decision List.
...
Give each decision a stable unique ID that does not change when the page refreshes.
...
Generate the Decision List as a live HTML page and open it with Lavish, per `~/.claude/rules/tools.md`'s Lavish conventions (open once, then edit-and-poll, never re-open).
Poll the Lavish page for John's responses.
When John answers a decision:

1. record the answer in the appropriate story or project artifact;
2. remove or resolve the captain hold as permitted by the governing workflow;
