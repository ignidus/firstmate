...
Keep each registry description useful for identifying the project, but keep delivery posture, John-private state, and detailed project knowledge in their existing designated homes.
...
If the owning second mate cannot accept the route, report that concrete blocker or obtain an explicit John redirection rather than silently duplicating the project in the main home.
...
The registry records the project's standing posture, which is John's default for the work rather than any task's answer; `AGENTS.md` section 7 owns how each task's concrete mode and yolo are resolved at intake and passed explicitly to the brief, the spawn, and any promotion.
...
`no-mistakes-prod-only` is the default for a newly added or created remote-backed project when John specifies nothing, and a project with no remote defaults to `local-only`.
State that resolved default while confirming the source, local name, and posture instead of asking John to choose from scratch, and record a flat mode instead whenever they ask for one.
...
Default it off for every project and every posture, and enable it only on John's explicit instruction.
`AGENTS.md` section 7 owns the complete authority boundary and exceptions when it is on.

## Add or clone an existing project

Confirm the source URL, local project name, delivery posture, and autonomy posture, stating the resolved default for each rather than asking John to invent one.
...
Before making that remote change, propose the repository name, owner or organization, visibility, and delivery posture, defaulting visibility to private and the posture to `no-mistakes-prod-only`, then obtain John's explicit consent for those exact values; a stated default never replaces that consent.
...
First obtain John's explicit removal decision, then inspect the current digest and authoritative repositories for in-flight or queued work, registered secondmate clones, linked worktrees, dirty files, unpushed commits, and any other unlanded work.
...
Once that preflight confirms none of the above and John's approval is concrete, AGENTS.md hard rule 1's John-approved project operation exception authorizes firstmate to remove the clone directly and update its registry entry to match.
