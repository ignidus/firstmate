...
name: afk
description: >-
  Enter away-mode supervision when John invokes /afk, says they are going afk, `state/.afk` exists, an incoming message starts with `FM_INJECT_MARK`, or any `state/.subsuper-*` marker is involved.
  It sets a durable away-mode flag so the sub-supervisor daemon can self-handle routine wakes and escalate John-relevant events plus bounded declared-external-wait rechecks as batched digests during walk-away stretches, then exits automatically when any real unmarked message returns firstmate to full per-wake responsiveness.
user-invocable: true
metadata:
  internal: true
---
...
tradeoff **consented** and **explicit**: John is stepping away, so the
...
2. **Ensure the sub-supervisor daemon is running as a tracked background process.**
   ...
   - **Harness WITH a native in-pane tracked-background tool** (e.g. claude's
     ...
     The launcher still owns lifecycle state and records the no-terminal mode, while the daemon inherits and auto-discovers John pane.
   ...
   - **Harness WITHOUT one** (e.g. pi): run `bin/fm-afk-launch.sh start`. It is
     ...
     a detached tmux session), records its exact id, and passes John pane
     in as `FM_SUPERVISOR_TARGET` so the daemon injects into the captain, not its
     own new pane. **Never manufacture a terminal by splitting John's
     active pane** (`herdr pane split`): a split co-tenants the tab and visibly
     shrinks John's pane (docs/herdr-backend.md "Away-mode supervisor
     support").
...
- A message **without** the current operational prefix or a legacy bare marker, and **not** starting with `/afk` -> John is back.
  Run `bin/fm-afk-return.sh` before acting on the message that brought John back.
...
a false exit is self-correcting (John re-runs `/afk`).
...
A PR ready for merge or a needs-decision finding keeps the same configured authority and exceptions from `AGENTS.md` section 7, while anything requiring John still waits for John's explicit word.
...
In afk mode the composer guard is belt-and-suspenders (no human is typing), but it protects against the race window between John returning and their message landing, a dead shell, and the daemon's own previous injection sitting unsent.
...
The classification predicates (the John-relevant verb set, declared-pause vocabulary, signal/stale tests, and fleet-scan) live in the shared `bin/fm-classify-lib.sh`, the same library the always-on watcher uses for its own triage when afk is off, so the two modes apply one identical policy.
...
- `signal` with a terminal captain verb (`done:`, `needs-decision:`, `blocked:`, or `failed:`) -> escalate.
  ...
  Other signals with no John-relevant status -> self-handle.
...
- `stale` with a terminal status or bare legacy John-relevant line -> escalate.
...
- `heartbeat` -> self-handle. The daemon runs its own cheap bash fleet scan
  ...
  John-relevant status line the per-wake classifier might miss.
...
- **Busy and composer guards on the supervisor pane** - before injecting, the daemon runs the detected-primary-harness rendered busy guard and reads `fm_backend_composer_state` directly.
  ...
  Every other result preserves the buffer for retry, so the daemon never merges its digest into John's half-typed line or types it into a shell.
...
- **Dedupe across signal/stale/scan** - `classify_signal` and terminal `classify_stale` paths check the seen-status marker before escalating, so a John-relevant status escalated by one path is not re-escalated by another in the same digest.
