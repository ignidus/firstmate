...
Home-domain captain preferences go to `data/captain.md`, cross-domain shared captain preferences go to the primary home's `data/captain-shared.md`, fleet-local operational facts and gotchas go to home-local `data/learnings.md` or, when still true but not worth loading every session, to the out-of-budget `data/learnings-archive.md` behind that index, project-intrinsic knowledge goes through normal crewmate delivery into that project's committed `AGENTS.md`, and task-scoped notes or undone next steps go to the backlog.
The [operational learnings reference](configuration.md#operational-learnings-datalearningsmd) owns the loaded-index and archive surface definition, and the `stow` skill owns the archiving procedure.
