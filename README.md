...
Launching a supported harness inside it instantiates your first mate - and makes you John.
...
- **Strict project boundary** - the first mate is read-only over your projects except for the narrow guarded and John-approved operations authorized by [hard rule 1](AGENTS.md#1-identity-and-prime-directives), including fleet sync's guarded safe branch pruning; crewmates make every other project change behind the configured merge authority.
...
# Minutes later:
  ...
  PR ready for review, John: https://github.com/you/xyz/pull/42
...
| Skill              | What it does                                                                                                                                  |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `/afk`             | Enter away-mode supervision: the sub-supervisor self-handles routine notifications in bash, escalates John-relevant events and bounded declared-external-wait rechecks as batched digests, and actively alerts if delivery gets stuck while you step away |
| `/ahoy`            | Recap visible session events since the prior real John message plus visibly unanswered John decisions, falling back to Bearings when invoked as the session's first real John message |
...
| `/cyb-sweep`       | Sweep the full open CYB Jira backlog behind a mandatory dry-run approval gate, classify every story by its own technical reading rather than Jira's stored Blocked-by field, and surface every decision John needs to make on a live Lavish page |
